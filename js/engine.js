const Sovereign = {
    ...
    sync() {
        ...
        let tr = ""; 
        if(clsKey === "rogue" && lv >= 11) tr += "• Reliable Talent (Min 10 on Proficient Skills)\n";
        this.state.custom.forEach(c => tr += `• ${c.n}\n`);
        this.state.items.forEach(i => tr += `• ${MagicItemsList[i]?.name || i}\n`);
        this.state.feats.forEach(f => tr += `• ${FeatsList[f]?.name || f}\n`);
    ...
    toggleSidebar() {
        ...
        document.getElementById('mobile-toggle').innerText = sb.classList.contains('active') ? "✕ Close" : "☰ Edit Hero";
    },
